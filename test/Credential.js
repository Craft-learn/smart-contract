...
describe("SoulboundCredential", function () {
  ...
  async function deploySoulboundCredential() {
    // Contracts are deployed using the first signer/account by default
    const [owner, student1, student2, student3] = await ethers.getSigners();

    const SoulboundCredential = await ethers.getContractFactory(
      "SoulboundCredential"
    );
    const soulboundCredential = await SoulboundCredential.deploy();

    return { owner, student1, student2, student3, soulboundCredential };
  }

  describe("Deployment", function () {
    it("Should set the right ERC721 name", async function () {
      const { soulboundCredential } = await loadFixture(
        deploySoulboundCredential
      );

      expect(await soulboundCredential.name()).to.equal(
    ...
    it("Should set the right ERC721 symbol", async function () {
      ...
      expect(await soulboundCredential.symbol()).to.equal("CRAFT-CRED");
    });
  });

  describe("Credential Creation", function () {
    it("Should register students", async function () {
      const { student1, student2, soulboundCredential } = await loadFixture(
        deploySoulboundCredential
      );

      const studentAddresses = [student1.address, student2.address];

      const tx = await soulboundCredential.registerStudents(studentAddresses);
      const receipt = await tx.wait();

      const checkStatus1 = await soulboundCredential.isStudent(
        student1.address
      );
      const checkStatus2 = await soulboundCredential.isStudent(
        student2.address
      );
  ...
  describe("Credential Minting", function () {
    it("Should mint a credential", async function () {
      const { student1, soulboundCredential } = await loadFixture(
        deploySoulboundCredential
      );
      ...
      const registrationTx = await soulboundCredential.registerStudents([
        student1.address,
      ]);
      await registrationTx.wait();

      const tx = await soulboundCredential
        .connect(student1)
        .mintCredential(courseName, tokenUri, learnerName);
      const receipt = await tx.wait();

      expect(await soulboundCredential.ownerOf(1)).to.equal(student1.address);
    });
  });
